...
const NoticeProfilePage = () => {
    ...
    const fetchNoticeData = async () => {
      try {
        const response = await fetch(
          `http://localhost:8080/api/admin/getnotice`,
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          ...
          setError(data.message || "Failed to load notice");
        }
        setLoading(false);
      } catch (err) {
  ...
  if (!notice)
    return <div className="text-center text-lg">Notice not found</div>;
...
export default NoticeProfilePage;
